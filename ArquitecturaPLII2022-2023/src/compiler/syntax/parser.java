...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Tue Mar 28 20:32:01 BST 2023
  */
  ...
  public final java_cup.runtime.Symbol CUP$parser$do_action(
    ...
    throws java.lang.Exception
    {
      ...
      switch (CUP$parser$act_num)
		...
		CasoAlt caso = (CasoAlt)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		...
		CasosAlternativa cA = (CasosAlternativa)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		int casoleft = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).left;
		int casoright = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).right;
		CasoAlt caso = (CasoAlt)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		...
		int altleft = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-7)).left;
		int altright = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-7)).right;
		Token alt = (Token)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-7)).value;
		...
		CasosAlternativa cA = (CasosAlternativa)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-2)).value;
		...
		PorDefecto pD = (PorDefecto)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
						...
						/*		// Comprobar que todos los casos tengan un tipo compatible con la expresion 
							    for (CasoSwitch caso : cS.getCasos()) {
							        if (!caso.getExpresion().getType().equals(e.getType())) {
							            semanticErrorManager.semanticFatalError("Error en linea " + caso.getLine() + ": Caso no compatible con la expresion");
							        }
							    }
							
							    if (d != null && !d.getSentencias().isEmpty() && !d.getSentencias().get(0).getIntermediateCode().getLast().getTemporals().isEmpty()) {
							        semanticErrorManager.semanticFatalError("Error en linea " + d.getLine() + ": Default no puede tener expresiones");
							    }
								*/
								...
								//generar codigo intermedio de la expresion
								...
								for (CasoAlt caso : cA.getCasos()) {
							        ...
							        // Crear label para el caso
							        LabelIF labelCaso = lf.create();							        
							        							        
							        // Generar cuadrupla para saltar a la label del caso si el numero del caso coincide con la expresion 
							        cb.addQuadruple("BEQ", tempExp, caso.getTemporal(), labelCaso); 							        		        
							        
							        // Si no hay coincidencia, continuar con el siguiente caso
							        cb.addQuadruple("BR", lf.create());
							        
							        // Generar codigo intermedio para el bloque del caso 
							        cb.addQuadruple("INL", labelCaso);
							        cb.addQuadruples(caso.getBloque().getIntermediateCode());
							        
							        // Al terminar, saltar al final de la sentencia S
							        cb.addQuadruple("BR", lf.create());
							    }
							    
							    // Si no hay coincidencia, se ejecuta la sentencia porDefecto (si existe) 
							    if (pD != null) {
							        LabelIF labelDefault = lf.create();
							        cb.addQuadruple("INL", labelDefault);
							        cb.addQuadruples(pD.getBloque().getIntermediateCode());
							        cb.addQuadruple("BR", lf.create());
							    }
							    
							    // Label para el final de la sentencia switch 
							    cb.addQuadruple("INL", lf.create());
