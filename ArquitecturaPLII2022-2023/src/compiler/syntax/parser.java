...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Sat Mar 25 19:34:50 GMT 2023
  */
    ...
    throws java.lang.Exception
    {
      ...
      switch (CUP$parser$act_num)
		...
		OpcionesEscribeEnt opEscEnt = (OpcionesEscribeEnt)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-2)).value;
							...
							String str = String.valueOf(opEscEnt.getOpciones().getTemporal());
							LabelIF lb = lf.create();
							TemporalIF temp = tf.create();
							
							cb.addQuadruple("ESCRIBE",temp,lb);
							
							// Guardamos la cadena en la lista para recuperarla al final del programa
							listaCadenas.add(new Quadruple("CADENA",new Label(str),lb));
		...
		DeclaracionConstante dec = (DeclaracionConstante)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		int sCleft = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).left;
		int sCright = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).right;
		SeccionConstantes sC = (SeccionConstantes)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		...
		int ax2left = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).left;
		int ax2right = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).right;
		Axiom2 ax2 = (Axiom2)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
			  	...
			  	for(ScopeIF s : scopeManager.getAllScopes()){
			  		...
			  		int dirRA = 4; // posiciones reservadas dentro del RA (valor de retorno + e. de control + estado maqina + e. acceso)
			  		...
			  		for(SymbolIF sim : s.getSymbolTable().getSymbols()){			  			
			  			if(sim instanceof SymbolVariable){
			  				...
			  				((SymbolVariable)sim).setAddress(dirRA);
			  				dirRA += sim.getType().getSize();
			  			}
			  		}			  		
			  		for(TemporalIF temp : s.getTemporalTable().getTemporals()){
			  			if(temp instanceof Temporal){
			  				...
			  				((Temporal)temp).setAddress(dirRA);
			  				dirRA += ((Temporal)temp).getSize();
			  			}
			  		}
			  	}
			  	...
			  	TemporalFactory tf = new TemporalFactory(scope);
			  	TemporalIF temp = tf.create();		  			
			    int tamanoRA = scope.getSymbolTable().getSize() + scope.getTemporalTable().getSize() + 4;	
			    			     					  
			  	cb.addQuadruple("STARTGLOBAL"); // prepara el RA global
			  	cb.addQuadruple("PUNTEROGLOBAL",temp,tamanoRA); // posiciona el SP segun el tamano del RA
