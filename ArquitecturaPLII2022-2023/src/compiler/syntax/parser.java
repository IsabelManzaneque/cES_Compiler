...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Sat Mar 04 20:28:32 GMT 2023
  */
    ...
    throws java.lang.Exception
    {
      ...
      switch (CUP$parser$act_num)
		...
		Expresion exp = (Expresion)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
							...
							// ref puede ser una variable o un acceso a vector							
							if(r instanceof AccesoVector){
								
							}else{								
								// comprobar si ref existe 
								String refName = (String)r;
								if(!scopeManager.containsSymbol(refName)){
									semanticErrorManager.semanticFatalError("Error en linea " + a.getLine() + ": El simbolo '" + refName + "' no ha sido declarado");							
								}
								SymbolIF symbol = scopeManager.searchSymbol(refName);	
								
								// comprobar que ref no es una constante o una funcion
								if(symbol instanceof SymbolConstant){
									semanticErrorManager.semanticFatalError("Error en linea " + a.getLine() + ": No es posible modificar una constante");
								}							
								if(symbol instanceof SymbolFunction){
									semanticErrorManager.semanticFatalError("Error en linea " + a.getLine() + ": No es posible asignar a una funcion");
								}
								if(symbol.getType() instanceof TypeArray){
									semanticErrorManager.semanticFatalError("Error en linea " + a.getLine() + ": No es posible hacer asignacion directa a vector");
								}							
		...
		LlamadaFuncion fun = (LlamadaFuncion)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
				...
				if(!(symbolFun.getType() instanceof TypeSimpleEntero)){
					semanticErrorManager.semanticFatalError("Asignacion de tipos incompatibles");
				}	
